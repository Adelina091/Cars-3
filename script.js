...
function insertToInput(name){
    search.value = name.textContent
    suggestions.innerHTML = ""
    suggestions.classList.remove("suggestions-active")
    suggestions.classList.add("suggestions-inactive")
}

search.addEventListener("input", event => {
    if(event.target.value){
        const carNames = carsArray.map(car => car.name);
        carNames.forEach(name => {
            if (name[0].toLowerCase().includes(event.target.value.toLowerCase())) {
                suggestions.classList.remove("suggestions-inactive")
                suggestions.classList.add("suggestions-active")
                suggestions.innerHTML = ""
                let new_suggestion = document.createElement("li")
                new_suggestion.innerHTML = `
                    <li onclick = "insertToInput(this)">${name}</li>
                `
                suggestions.appendChild(new_suggestion)
